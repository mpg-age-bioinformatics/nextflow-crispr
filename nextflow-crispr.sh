...
nextflow run ${ORIGIN}nf-fastqc ${FASTQC_RELEASE} -params-file ${PARAMS} -profile ${PROFILE} 2>&1 | tee ${LOGS}/nf-fastqc.log ${LOGS_}/nf-fastqc.log && sleep 1 #& FASTQC_PID=$!
...
nextflow run ${ORIGIN}nf-mageck ${MAGECK_RELEASE} -params-file ${PARAMS} -entry magecku -profile ${PROFILE} 2>&1 | tee ${LOGS}/nf-mageck-u.log ${LOGS_}/nf-mageck-u.log && sleep 1 #& MAGECK_U_PID=$!

echo "$(date '+%Y-%m-%d %H:%M:%S'): mageck pathway"
nextflow run ${ORIGIN}nf-mageck ${MAGECK_RELEASE} -params-file ${PARAMS} -entry mageck_pathway -profile ${PROFILE} 2>&1 | tee ${LOGS}/nf-mageck-pathway.log ${LOGS_}/nf-mageck-pathway.log && sleep 1 #& MAGECK_PATHWAY_PID=$!

echo "$(date '+%Y-%m-%d %H:%M:%S'): mageck plot"
nextflow run ${ORIGIN}nf-mageck ${MAGECK_RELEASE} -params-file ${PARAMS} -entry mageck_plot -profile ${PROFILE} 2>&1 | tee ${LOGS}/nf-mageck-plot.log ${LOGS_}/nf-mageck-plot.log && sleep 1 #& MAGECK_PLOT_PID=$!

echo "$(date '+%Y-%m-%d %H:%M:%S'): bagel"
nextflow run ${ORIGIN}nf-bagel ${BAGEL_RELEASE} -params-file ${PARAMS} -profile ${PROFILE} 2>&1 | tee ${LOGS}/nf-bagel.log ${LOGS_}/nf-bagel.log && sleep 1 #& BAGEL_PID=$!

echo "$(date '+%Y-%m-%d %H:%M:%S'): drugz"
nextflow run ${ORIGIN}nf-drugz ${DRUGZ_RELEASE} -params-file ${PARAMS} -profile ${PROFILE} 2>&1 | tee ${LOGS}/nf-drugz.log ${LOGS_}/nf-drugz.log && sleep 1 #& DRUGZ_PID=$!

echo "$(date '+%Y-%m-%d %H:%M:%S'): acer"
nextflow run ${ORIGIN}nf-acer ${ACER_RELEASE} -params-file ${PARAMS} -profile ${PROFILE} 2>&1 | tee ${LOGS}/nf-acer.log ${LOGS_}/nf-acer.log && sleep 1 #& ACER_PID=$!

echo "$(date '+%Y-%m-%d %H:%M:%S'): maude"
nextflow run ${ORIGIN}nf-maude ${MAUDE_RELEASE} -params-file ${PARAMS} -profile ${PROFILE} 2>&1 | tee ${LOGS}/nf-maude.log ${LOGS_}/nf-maude.log && sleep 1 #& MAUDE_PID=$!
...
nextflow run ${ORIGIN}nf-mageck ${MAGECK_RELEASE} -params-file ${PARAMS} -entry mageck_vispr -profile ${PROFILE} 2>&1 | tee ${LOGS}/nf-mageck-vispr.log ${LOGS_}/nf-mageck-vispr.log && sleep 1 #& MAGECK_VISPR_PID=$!

echo "$(date '+%Y-%m-%d %H:%M:%S'): mageck flute"
nextflow run ${ORIGIN}nf-mageck ${MAGECK_RELEASE} -params-file ${PARAMS} -entry mageck_flute -profile ${PROFILE} 2>&1 | tee ${LOGS}/nf-mageck-flute.log ${LOGS_}/nf-mageck-flute.log && sleep 1 #& MAGECK_FLUTE_PID=$!
